    ...
    <div className="space-y-6">
      ...
      {!configured && (
        <div className="flex items-start gap-2 rounded-apple-lg border border-amber-200 bg-amber-50 p-4 text-[13px] text-amber-800">
          ...
          <div>
            База не настроена. Добавь <code className="rounded bg-white/60 px-1">DATABASE_URL</code> в .env, подними Postgres с
            расширением <code className="rounded bg-white/60 px-1">vector</code> и выполни миграцию{' '}
            <code className="rounded bg-white/60 px-1">supabase/migrations/003_me.sql</code>.
          </div>
        </div>
      )}
