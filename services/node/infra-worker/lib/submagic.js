...
/**
 * Submagic has no documented credit-balance endpoint, so we can't pre-check.
 * Use `isInsufficientCreditsError(text)` to recognise the 402 response shape
 * and let the caller decide whether to skip the captioner step.
 */
export function isInsufficientCreditsError(text) {
  if (!text) return false;
  return /INSUFFICIENT_CREDITS|insufficient.*credit/i.test(String(text));
}
