...
echo "  3. cp .env.example .env && edit it. Suggested values:"
echo "       DOMAIN=\$(curl -s ifconfig.me).sslip.io"
echo "       POSTGRES_PASSWORD=\$(openssl rand -base64 32)"
echo "       ADMIN_PASSWORD=\$(openssl rand -base64 32)"
echo "       YTDLP_SERVICE_API_KEY=\$(openssl rand -hex 24)"
echo "     Plus your API keys: ANTHROPIC_API_KEY, DEEPGRAM_API_KEY, OPENAI_API_KEY,"
echo "     and TELEGRAM_BOT_TOKEN (gates /api/transcribe — required for prod)."
echo "  4. docker compose up -d --build"
echo
echo "App will be at https://\$DOMAIN/transcribe."
echo "Admin board at https://\$DOMAIN/admin (Basic Auth: admin / \$ADMIN_PASSWORD)."
