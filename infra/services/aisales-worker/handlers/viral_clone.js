...
async function stepHeygenSubmit(p) {
  ...
  const r = await heygen.submitVideo({
    ...
    avatarId: p.heygen_avatar_id,    // если NULL — lib возьмёт HEYGEN_DEFAULT_AVATAR_ID
...
async function stepHeygenPoll(p) {
  ...
  if (s.status === 'failed') {
    return failPipeline(p, `heygen failed: ${s.errorMessage || 'no detail'}`);
  }
...
async function stepSubmagicSubmit(p) {
  ...
  if (!r.ok) return failPipeline(p, `submagic submit: ${r.error}`);
...
async function stepSubmagicPoll(p) {
  ...
  if (!s.ok) return bumpPollAndReschedule(p, 'submagic', `submagic poll fetch: ${s.error}`);
...
async function failPipeline(p, reason) {
  ...
  if (p.tg_chat_id) {
    await sendText(p.tg_chat_id,
      ...
      `Причина: <code>${escapeHtml(String(reason).slice(0, 200))}</code>`).catch(() => {});
  }
