...
(function () {
  ...
  async function initAgentsImpl() {
    ...
    try {
      ...
      // Honest title — replace the baked-in "команда из 9 AI-продавцов"
      // with the real count: N IG prompts (typically 1) + 1 static
      // tg-agent card. Russian plural rule applied via pluralAgents().
      const titleEl = $('.page-h__title');
      if (titleEl) {
        const total = (prompts.length || 0) + 1;
        titleEl.textContent = 'Agents · команда из ' + total + ' ' + pluralAgents(total);
      }
    } catch (e) {
      grid.innerHTML = '';
