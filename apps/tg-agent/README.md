...
- **Чат с агентом-аналитиком** (`src/agent_chat.ts`): владелец пишет
  боту в личку обычный вопрос («что обсуждали в AICEX за сутки?»,
  «были ли возражения по цене?», «кому я не ответил?») — Claude
  (Sonnet) через tool-use сам поднимает данные (семантический поиск
  по памяти, сводки по чатам, журнал сообщений) и отвечает. Диалог
  многоходовый, `/reset` — начать заново. Гейтится `AGENT_CHAT_ENABLED`
  (по умолчанию вкл.) + `OWNER_TELEGRAM_ID`. Доступно только владельцу.
- **Форум-режим** (`src/forum.ts`, `AGENTS.md`, `ARCHITECTURE.md`):
  вывод бота переезжает в одну форум-группу, где каждая тема (ветка) —
  отдельный AI-агент (Контент / Продуктолог / Финансист / Ассистент /
  General). Отчёты раскладываются по веткам, в каждой ветке можно
  общаться с профильным агентом (контекст: роль + отчёты ветки +
  история ветки). Выключено по умолчанию; включается `FORUM_ENABLED` +
  `FORUM_GROUP_ID`. Настройка маршрутов без кода: `/route`, `/topics`,
  `/bindthread`, `/createtopics`.
