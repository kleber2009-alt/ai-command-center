...
import { buildCabinetLink } from './cabinet.js';
...
export async function handleCallback(cq) {
  ...
  if (data === 'menu:cabinet') {
    await ack(cq);
    await sendCabinetLink(chatId, user.id);
    return;
  }
  if (data === 'menu:run') {
...
async function handleCommand(msg, user, config) {
  ...
  switch (cmd) {
    ...
    case '/cabinet':  return await sendCabinetLink(chatId, user.id);
    case '/list':
...
function buildMainMenuKb(config) {
  ...
  return {
    inline_keyboard: [
      [{ text: '🖥 Открыть кабинет', callback_data: 'menu:cabinet' }],
...
// One-time deep link to the cabinet. Token TTL — 10 min, set via cabinet.js.
async function sendCabinetLink(chatId, userId) {
  const link = buildCabinetLink(CABINET_BASE_URL, userId, chatId);
  if (!link) {
    await sendText(chatId,
      'Кабинет ещё не сконфигурирован (нет CABINET_LINK_SECRET в env). Сообщи Илье.',
      { bot: BOT });
    return;
  }
  await sendText(chatId,
    '🖥 <b>Твой персональный кабинет</b>\n\n' +
    'Ссылка одноразовая, живёт 10 минут. После входа браузер запомнит тебя на 30 дней.\n\n' +
    'Что внутри:\n' +
    '• сегодняшняя выдача с метриками и оценками\n' +
    '• история всех запусков\n' +
    '• управление источниками без команд\n' +
    '• пороги и расписание',
    {
      bot: BOT,
      reply_markup: {
        inline_keyboard: [[{ text: '🖥 Открыть кабинет', url: link }]],
      },
    });
}
