...
const Body = z.object({
  ...
  minCarouselLikes: z.number().int().min(0).optional(),
  minCarouselComments: z.number().int().min(0).optional(),
...
export async function POST(req: NextRequest) {
  ...
  const config = await prisma.parserConfig.upsert({
    ...
    create: {
      ...
      minLikes: data.minLikes ?? 1000,
      minComments: data.minComments ?? 1000,
      minShares: data.minShares ?? 1000,
      minCarouselLikes: data.minCarouselLikes ?? 1000,
      minCarouselComments: data.minCarouselComments ?? 1000,
    ...
    update: {
      ...
      minCarouselLikes: data.minCarouselLikes,
      minCarouselComments: data.minCarouselComments,
